...
function getStateSize(room: { _serializer: { state: any; previousState: any; getFullState: () => any; }; }) {
...
// Actions
(<any>Room.prototype)._forceClientDisconnect = async function (sessionId: any) {
...
(<any>Room.prototype)._sendMessageToClient = async function (sessionId: any, type: any, data: any) {
    for (let i = 0; i < this.clients.length; i++) {
        if (this.clients[i].sessionId === sessionId) {
